...
The script [ssmBuild.m]() creates a model of the 4-DoF SSM that has been studied in [[2]](). The script [sd_ssm.m]() creates three `Transmission` instances that correspond to the robot's transmission systems. These objects can be added to [simINVD_ssm.slx]() for simulating the robot's inverse dynamics. The script [ssmMotorization.m]() creates a desired motion response for a selected joint of the 4-DoF SSM and, after running [simINVD_ssm.slx](), provides estimations for the torque requirements.
